...
<p>In this case, the Raspberry Pi Zero W (wireless) is used to program the MCU. This is an even more convenient and budget friendly method than using the FT232H used previously as a USB to UART modem, but for rerference, see <a href="http://catch22.eu/baremetal/lpc11xxgnu/">this tutorial</a> on programming the LPC1114 using this modem. So how does this work? The Raspberry Pi's GPIO's can be used to send and receive the JTAG Servial Wire Debug signals to and from the STM32, using the openocd software package. The advantage here, is that next to being budget friendly (the Raspberry Pi Zero Wireless retails as low as $10), the Pi itself acts both as compiler / debugger, and interfaces directly to the STM32. This, opposed to using a USB to UART modem, which requires connecting to the model via USB (and additional issues with drivers). <b>All subsequent commands below are executed on the Raspberry Pi itself</b>, either remotely via ssh making use of the wireless connection for headless setups (e.g. using Raspbian Lite), or directly via keyboard/mouse/monitor connected to the Pi.</p>

<h2>Setting up the GNU Toolchain</h2>
<?php include 'gcc-arm-none-eabi.php'; ?>
...
<p>There are some caveats on "older" Debian based versions of openocd, see <a href="http://catch22.eu/baremetal/openocd_sysfs_stm32/">this extensive tutorial</a> about obtaining the correct version.</p>
...
<img src="http://catch22.eu/baremetal/openocd_sysfs_stm32/raspbery_pi_openocd.jpg" alt="Wire diagram Raspberry Pi as SWD JTAG programmer for the STM32" height="100%" width="100%"><br>
...
<img src="http://catch22.eu/baremetal/openocd_sysfs_stm32/raspberry_as_programmer.jpg" alt="Wire diagram Raspberry Pi as SWD JTAG programmer for the STM32" height="100%" width="100%"><br><br>
